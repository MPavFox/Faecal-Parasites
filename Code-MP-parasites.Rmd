...
#Load parasite count
parasites <- read.csv("C:/Users/mp660/OneDrive - University of Exeter/PhD/Thesis_parasites/Data for Markdown/parasites_final_analyses.txt")

#Load Bison network metrics
networks = readRDS("C:/Users/mp660/OneDrive - University of Exeter/PhD/Thesis_parasites/Data for Markdown/for_bison/Bison_parasites2.RData")

#Load demographic file
census <- read.csv("C:/Users/mp660/OneDrive - University of Exeter/PhD/Thesis_injury/Data for Markdown/Census_Sep2020.csv")

#Directory for models
directory = "C:/Users/mp660/OneDrive - University of Exeter/PhD/Thesis_parasites/Data for Markdown/brms models/final/"

#Directory for edgelists
dir_edges = "C:/Users/mp660/OneDrive - University of Exeter/PhD/Thesis_parasites/Data for Markdown/"
...
- Richness: number of concurrent infections.

Measures of Sociality:
- Social status: agonistic observations from the three groups (males + females).
- Number of partners (degree): grooming and proximity separately.
...
#Age*rank
emm = emtrends(prev_coli,  pairwise ~ outrank_perc, var = "age", at = list(outrank_perc = c(10,100)))
confint(emm, level = .89)

#At average age = 10.6 years
#low-status (10%) median = 0.236, 89% CI = 0.0295, 0.435
#High-status (100%) median = -0.288, 89% CI = -0.5281, -0.056
...
prev_coliSNA_weakP <- readRDS(paste0(directory,"prev_coliSNA_weakP.rds"))#load model
...
prev_coliSNA_weakG <- readRDS(paste0(directory,"prev_coliSNA_weakG.rds"))#load model
...
#Save model
#saveRDS(prev_strongiSNA_weakP, file = paste0(directory,"prev_strongiSNA_weakP.rds"))
prev_strongiSNA_weakP <- readRDS(paste0(directory,"prev_strongiSNA_weakP.rds"))#load model
...
#Save model
#saveRDS(prev_strongiSNA_weakG, file = paste0(directory,"prev_strongiSNA_weakG.rds"))
prev_strongiSNA_weakG <- readRDS(paste0(directory,"prev_strongiSNA_weakG.rds"))#load model
...
#Save model
#saveRDS(prev_trichuSNA_weakP, file = paste0(directory,"prev_trichuSNA_weakP.rds"))
prev_trichuSNA_weakP <- readRDS(paste0(directory,"prev_trichuSNA_weakP.rds"))#load model
...
#Save model
#saveRDS(prev_trichuSNA_weakG, file = paste0(directory,"prev_trichuSNA_weakG.rds"))
prev_trichuSNA_weakG <- readRDS(paste0(directory,"prev_trichuSNA_weakG.rds"))#load model
...
tab_model(prev_trichuSNA_weakG,transform =  NULL, show.r2 = F, show.p = T, show.se = T, show.stat = T, show.icc = F, show.re.var = F, dv.labels = "Infection risk",
          show.ci = 0.89)
...
#Save model
#saveRDS(prev_coliSNA_str, file = paste0(directory,"final/prev_coliSNA_str.rds"))

prev_coliSNA_str <- readRDS(paste0(directory,"prev_coliSNA_str.rds"))#load model
...
#Save model
#saveRDS(prev_strongiSNA_str, file = paste0(directory,"prev_strongiSNA_str.rds"))
prev_strongiSNA_str <- readRDS(paste0(directory,"prev_strongiSNA_str.rds"))#load model
...
#Save model
#saveRDS(prev_trichuSNA_str, file = paste0(directory,"prev_trichuSNA_str.rds"))
prev_trichuSNA_str <- readRDS(paste0(directory,"prev_trichuSNA_str.rds"))#load model
...
for(k in 1:n_draws){
   ...
   temp$groom_weak = post_n$std_weak_groo
...
#Plot correlogram
library(corrplot)
cor.mat1 <- net_metrics %>% select(perc_rank,prox_weak, groom_weak, groom_str)
...
Figure S4: variation in age pre and post hurricane, and with social status
```{r}

#Is there enough data to test interactions?
#Age and hurricane
ggplot(parasites, aes(x = age, fill = post_hurricane)) + geom_density(alpha = 0.45, colour = "black")+ theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
#Seems that for age*post_hurricane its okay
